"""Plank world boots, tools are registered, agents can dispatch them."""
...
@pytest.mark.asyncio
async def test_plank_agent_dispatches_lookup_user():
    @scenario("plank_lookup")
    async def plank_lookup(world):
    ...
    # Construct against the plank world this time.
    from ensemble.scenario import World
    w = World("plank")
    # Re-run the scenario manually against the plank world; the
    # decorator defaults to noop, so use the underlying function.
    result = await _REGISTRY["plank_lookup"]("plank")
